...
GitHub Actions builds the application natively for macOS arm64, Windows x64, and Linux x64. A tag matching the version in `package.json` publishes the packages, generated release notes, and `SHA256SUMS.txt` to a GitHub Release.
...
5. Launch each packaged application and verify session discovery, model settings, a terminal command, external links, and shutdown.

Tags that do not exactly match `package.json` fail before packaging. Versions with a prerelease suffix, such as `v0.10.0-beta.1`, create a GitHub prerelease. Run the workflow manually from the Actions tab to test all packages without creating a release.
