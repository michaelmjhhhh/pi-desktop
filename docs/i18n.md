...
Pi Desktop's interface uses a small internal i18n layer. It is intentionally kept
...
Traditional Chinese browser locales (`zh-TW`, `zh-HK`, `zh-MO`, and
`zh-Hant` variants) resolve to `zh-TW`. Bare `zh`, Simplified Chinese regions,
and `zh-Hans` variants resolve to `zh-CN`.
...
/** Pi Desktop Japanese language package. */
...
Import the package and add it to `localePlugins` in `lib/i18n/registry.ts`:

```ts
import { jaLocale } from "./messages/ja";

const localePlugins: LocalePlugin[] = [enLocale, zhCNLocale, zhTWLocale, jaLocale];
```

The label is displayed in the language selector. `getSupportedLocales()`
automatically exposes packages in this list to the UI.

### 3. Update Locale-Specific Types If Needed

The built-in `Locale` type currently lists `en`, `zh-CN`, and `zh-TW` for
compile-time safety. When adding a built-in language, update that union in
`lib/i18n/types.ts` and update any locale validation that intentionally lists
the built-in identifiers.

### 4. Add Tests

Extend `lib/i18n/registry.test.mjs` when browser-language detection or locale
listing changes. Extend `lib/i18n/format.test.mjs` when interpolation or
...
1. Add a stable key to every built-in package in `lib/i18n/messages/`
   (`en.ts`, `zh-CN.ts`, and `zh-TW.ts`).
...
Pi Desktop is a user-facing web interface. Internationalization is therefore an
...
support should lower the barrier to using Pi Desktop, not be treated as a measure
