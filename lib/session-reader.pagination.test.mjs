...
const { sliceActiveBranch, buildSessionContext, buildSessionNavigation } = await jiti.import("./session-reader.ts");
...
test("navigation covers the full branch independently of the initial history page", () => {
  const entries = linearChain(300);
  const page = buildSessionContext(entries, "e299", { tail: 50 });
  const navigation = buildSessionNavigation(entries, "e299");
  assert.equal(page.messages.length, 50);
  assert.equal(navigation.length, 300);
  assert.equal(navigation[0].entryId, "e0");
  assert.equal(navigation.at(-1).entryId, "e299");
  assert.equal(buildSessionNavigation(entries, "e19").length, 20);
  assert.deepEqual(buildSessionNavigation(entries, null), []);
});

test("navigation excludes payloads and process text, retaining bounded final replies", () => {
  const entries = linearChain(3);
  entries[0].message.content = [{ type: "text", text: "question" }, { type: "image", data: "secret-image" }];
  entries[1].message.content = [
    { type: "text", text: "process text" },
    { type: "thinking", thinking: "private thinking" },
    { type: "toolCall", id: "call", name: "bash", arguments: { command: "secret command" } },
    { type: "text", text: "answer".repeat(1000) },
  ];
  entries[2].message = { role: "toolResult", content: [{ type: "text", text: "tool output" }] };
  const navigation = buildSessionNavigation(entries, "e2");
  assert.equal(navigation.length, 2);
  assert.equal(navigation[0].message.content[0].text, "question");
  assert.equal(navigation[1].message.content[0].text.length, 2000);
  const serialized = JSON.stringify(navigation);
  for (const excluded of ["secret-image", "private thinking", "secret command", "process text", "tool output"]) {
