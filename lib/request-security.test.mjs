...
test("rejects LAN same-origin requests when Next.js uses an internal localhost URL", async () => {
  const { isApiRequestAllowed } = await loadSubject();
...
test("allows IPv6 loopback but rejects remote hostnames", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  ...
  assert.equal(isApiRequestAllowed(configured), false);
});
...
test("rejects cross-site exports including user-initiated navigations", async () => {
  const { isApiRequestAllowed } = await loadSubject();
...
test("rejects a mismatched scheme even with proxy headers", async () => {
  const { isApiRequestAllowed } = await loadSubject();
